...
--}}
@php
$mainUrl = config('app.url').'/'.config('dotenveditor.route')['prefix'];
@endphp
@section('content')
  ...
  new Vue({
    ...
    methods: {
      loadEnv: function(){
        ...
        $.getJSON("{{ $mainUrl }}/getdetails", function(items){
          vm.entries = items;
        });
      },
      ...
      showBackupDetails: function(timestamp, formattedtimestamp){
        ...
        var vm = this;
        $.getJSON("{{ $mainUrl }}/getdetails/" + timestamp, function(items){
          vm.details = items;
