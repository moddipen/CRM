                ...
                <div class="card panel-primary">
                    <div class="card-heading">
                        <h3 class="card-title">
                            ...
                            Editing Role : <p class="user_name_max">{!! $editRole->name!!}</p>
                        </h3>
                    ...
                    <div class="card-body">
                        <!--main content-->
                        {!! Form::model($editRole, ['url' =>route('role.update',$editRole->id ), 'method' => 'put', 'class' => 'form-horizontal','id'=>'commentForm', 'enctype'=>'multipart/form-data','files'=> true]) !!}
                        {{ csrf_field() }}
                        <!-- CSRF Token -->
                                ...
                                <div class="tab-content">
                                    <div class="tab-pane " id="tab1">
                                        ...
                                        <div class="form-group {{ $errors->first('permission', 'has-error') }}">
                                            <div class="row">
                                                <label for="first_name" class="col-sm-2 control-label">Permission *</label>
                                                <div class="col-sm-10">
                                                    @foreach($permission as $val)
                                                        @if( $editRole->hasPermissionTo($val))
                                                        <input id="name" name="permissions[]" type="checkbox"
                                                           placeholder="Name" class="flat-red" checked
                                                           value="{{$val->id}}"/>&nbsp;&nbsp;{{$val->name}}<br/><br/>
                                                        @else
                                                        <input id="name" name="permissions[]" type="checkbox"
                                        ...
                                        <button align = 'center' type="submit" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="left" data-original-title="Update Role" id="delButton">Update</button>
                                    </div>
